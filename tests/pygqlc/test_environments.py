...
def test_enter_environ(gql):
    ...
    with gql.enterEnvironment("test"):
        ...
        assert context_env == "test", "environment should change inside WITH statement"
...
def test_change_url_environment(gql):
    ...
    assert new_url == test_url, "URL should set on current environment as default"
...
def test_change_wss_environment(gql):
    ...
    assert new_url == test_url, "WSS URL should set on current environment as default"
...
def test_change_post_timeout(gql):
    ...
    assert (new_post_timeout == test_post_timeout) and (
        post_timeout != new_post_timeout
