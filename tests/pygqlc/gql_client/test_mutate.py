...
def test_mutate_no_errors(gql):
    ...
    assert errors == [], "There should NOT be errors on this mutation"
...
def test_mutate_var(gql):
    ...
    assert py_.get(data_1, "result.active") == True, "ACTIVE should be set to True"
    assert py_.get(data_2, "result.active") == False, "ACTIVE should be set to False"


def test_bad_mutate_doc(gql):
    _, errors = gql.mutate(mutations.bad_author_create)
    assert len(errors) > 0, "Mutation SHOULD contain errors (bad syntax)"
