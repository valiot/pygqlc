...
def test_subscribe_gracefully_handles_missing_ws_url_and_environment():
    """OPS-3496: subscribe() and _new_conn must never raise (AttributeError on env=None
    or TypeError on ws_url=None) when no WSS is configured; must return None and log
    cleanly. This prevents thread death in _sub_routing_loop and production crashes.
    """
    Singleton._instances.pop(GraphQLClient, None)
    gql = GraphQLClient()
    with _capture_logs() as records:
        unsub = gql.subscribe("subscription { test }")
    assert unsub is None, (
        "subscribe must return None (not raise) when no ws_url configured"
    )
    assert any(level == LogLevel.ERROR for level, msg in records), (
        "expected at least one ERROR log on failure"
    )
